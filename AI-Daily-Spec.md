...
| 路由 | Vue Router | 4.x | |
| HTTP 客戶端 | Axios | 1.x | 含攔截器封裝 |
| UI 元件庫 | Naive UI | 2.x | 支援 Dark/Light |
| CSS 預處理器 | SCSS | — | BEM 命名規範 |
...
| 元件名稱 | 路徑 | 說明 |
|---------|------|------|
| `AppTopNav` | `components/layout/AppTopNav.vue` | 頂部導覽列 |
| `AppToolbar` | `components/layout/AppToolbar.vue` | 搜尋、Tag 篩選列 |
| `ArticleCard` | `components/article/ArticleCard.vue` | 文章卡片 |
| `ArticleFeed` | `components/article/ArticleFeed.vue` | 文章列表（含無限捲動） |
| `AiSummaryPanel` | `components/ai/AiSummaryPanel.vue` | 右側 AI 快速預覽 |
| `AiDeepReport` | `views/AiDeepReport.vue` | 深度報告頁 |
| `ImpactMeter` | `components/ai/ImpactMeter.vue` | 影響力評估進度條 |
| `TagFilter` | `components/common/TagFilter.vue` | Tag 篩選元件 |
| `ThemeToggle` | `components/common/ThemeToggle.vue` | 主題切換按鈕 |
| `BookmarkList` | `components/bookmark/BookmarkList.vue` | 今日書籤清單 |
| `StatsCard` | `components/dashboard/StatsCard.vue` | 統計數字卡片 |
| `LoadingSkeleton` | `components/common/LoadingSkeleton.vue` | 載入中骨架屏 |
...
│   │       └── AppToolbar.vue
│   ├── composables/
│   │   ├── useArticles.ts            # 文章列表邏輯
│   │   ├── useAiSummary.ts           # AI 總結邏輯（含 SSE）
│   │   ├── useBookmarks.ts
